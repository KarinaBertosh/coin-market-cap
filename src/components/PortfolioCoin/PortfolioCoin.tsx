...
import { KEY_LS, getPricesWithoutComma, getCoinsTotalAmount, renderPriceWithDollarAndCurrency } from '../../utils/default';
...
export const PortfolioCoin = ({ coin }: IPortfolioCoinProps) => {
  ...
  return (
    <div className="coin-block" key={key} >
      {symbol}:
      <p className="price">
        {renderPriceWithDollarAndCurrency(getCoinsTotalAmount([coin]))}
      </p>
      <Button
        className="m-r-10"
        onClick={deleteCoin}
